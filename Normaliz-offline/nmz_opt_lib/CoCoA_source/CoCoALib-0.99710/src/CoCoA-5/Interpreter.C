...
vector<RingElem> RuntimeEnvironment::evalArgAsListOfRingElem(const ring &ring, const Argument &arg) {
...
vector<RingElem> RuntimeEnvironment::evalRVAsListOfRingElem(const boost::intrusive_ptr<const RightValue> rightOp, const Argument &arg) {  
	const intrusive_ptr<const LIST> list = boost::dynamic_pointer_cast<const LIST>(rightOp);
	return toRingElemList(list, arg.exp, ringForList(list));
}
