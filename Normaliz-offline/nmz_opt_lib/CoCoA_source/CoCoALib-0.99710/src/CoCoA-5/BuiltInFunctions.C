...
DECLARE_STD_BUILTIN_FUNCTION(SystemCommand, 1)
{
  ...
  if (std::system(nullptr) == 0)
    throw RuntimeException("low-level \"system\" command unavailable", invocationExpression);
...
DECLARE_STD_BUILTIN_FUNCTION(ascii, 1) {
  ...
  case 1: {
    intrusive_ptr<LIST> l(new LIST());
		...
		BOOST_FOREACH(const unsigned char c, s)
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/CoCoA-5/BuiltInFunctions.C,v 1.304 2020/02/20 15:26:17 abbott Exp $
