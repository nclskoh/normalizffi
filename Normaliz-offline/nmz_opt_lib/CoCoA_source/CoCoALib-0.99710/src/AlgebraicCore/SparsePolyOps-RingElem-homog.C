...
namespace CoCoA
{
  ...
  RingElem HomogCompt(ConstRefRingElem f, long d)
  {
    ...
    if (!IsSparsePolyRing(P)) CoCoA_ERROR(ERR::NotElemSparsePolyRing, "HomogCompt");
    if (GradingDim(P) != 1) CoCoA_ERROR("GradingDim must be 1", "HomogCompt");
    if (d < 0) CoCoA_ERROR(ERR::NotNonNegative, "HomogCompt");
    RingElem form(P);
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/SparsePolyOps-RingElem-homog.C,v 1.2 2020/03/11 17:06:55 abbott Exp $
