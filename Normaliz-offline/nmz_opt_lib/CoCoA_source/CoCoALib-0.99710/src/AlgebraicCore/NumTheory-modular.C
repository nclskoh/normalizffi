...
// ???BUG NYI: use machine int code if possible???
  ...
  {
    if (IsNegative(modulus) || !IsSignedLong(modulus) || AsSignedLong(modulus) < 2) CoCoA_ERROR(ERR::BadModulus, "KroneckerSymbol");
  ...
  {
    if (modulus < 2) CoCoA_ERROR(ERR::BadModulus, "KroneckerSymbol");
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/NumTheory-modular.C,v 1.1 2020/01/26 14:14:31 abbott Exp $
