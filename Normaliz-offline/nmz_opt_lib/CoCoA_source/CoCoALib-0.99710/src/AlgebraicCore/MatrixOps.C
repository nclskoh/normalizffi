...
namespace CoCoA
{
  ...
  {
    if (i < 0 || i >= NumRows(M)) CoCoA_ERROR(ERR::BadIndex, "GetRow(M,i)");
  ...
  {
    if (j < 0 || j >= NumCols(M)) CoCoA_ERROR(ERR::BadIndex, "GetCol(M,j)");
...
//???      CoCoA_ASSERT(n > 0);
  ...
  bool IsZeroDet(const ConstMatrixView& M)
  {
    ...
    if (NumRows(M) != NumCols(M))
      CoCoA_ERROR(ERR::NotSquareMatrix, "IsZeroDet(Mat)");
    return IsZero(det(M));
  }
...
///JAA???return AdjByDetOfMinors(M);
  ...
  RingElem DetOfSmallMat(ConstMatrixView M)
  {
    ...
    if (NumCols(M) != n) CoCoA_ERROR(ERR::NotSquareMatrix, "DetOfSmallMat");    
    if (n > 5) CoCoA_ERROR(ERR::ArgTooBig, "DetOfSmallMat");
    switch (n)
    {
    case 0:
      return one(RingOf(M));
    ...
    case 5:
      return det5x5(M);
    default:
      CoCoA_ERROR(ERR::ShouldNeverGetHere, "DetOfSmallMat");
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/MatrixOps.C,v 1.11 2020/02/27 10:53:47 abbott Exp $
// $Log: MatrixOps.C,v $
