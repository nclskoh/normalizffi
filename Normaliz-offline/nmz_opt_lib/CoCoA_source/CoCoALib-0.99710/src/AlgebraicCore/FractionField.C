...
namespace CoCoA
{
  ...
  const FractionFieldBase* FractionFieldPtr(const ring& R, const char* const FnName)
  {
    ...
    if (ptr == nullptr) CoCoA_ERROR(ERR::NotFracField, FnName);
    return ptr;
  }
...
//    CancelFactors(rawlhs);
  ...
  void FractionFieldImpl::InducedHomImpl::myApply(RingElemRawPtr rawimage, RingElemConstRawPtr rawarg) const
  {
    ...
    if (IsZero(ImDen))
      CoCoA_ERROR(ERR::BadPartialRingHomArg, "FractionFieldImpl::InducedHomImpl::myApply");
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/FractionField.C,v 1.51 2020/02/11 16:56:40 abbott Exp $
