...
namespace CoCoA
{
  ...
  matrix LinKerZZ(ConstMatrixView M)
  {
    ...
    if (!IsZero(characteristic(R))) CoCoA_ERROR("char not 0", "LinKerZZ");
    ...
    vector<BigRat> row(ncols);
    for (int j=0; j < ncols; ++j)
    {
      for (int i=0; i < nrows; ++i)
        row[i] = ConvertTo<BigRat>(M(i,j)); // may throw
      const BigInt D = CommonDenom(row);
      for (int i=0; i < nrows; ++i)
        M_ZZ[j][i] = num(row[i])*(D/den(row[i]));
    }
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/MatrixOps-LinKerZZ.C,v 1.1 2020/02/28 08:56:18 abbott Exp $
