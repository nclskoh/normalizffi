...
double LogFactorial(const BigInt& N)
{
  ...
  {
    if (N < 0)
      CoCoA_ERROR(ERR::NotNonNegative, "primorial(BigInt)");
    unsigned long n;
    if (!IsConvertible(n, N))
      CoCoA_ERROR(ERR::ArgTooBig, "primorial(BigInt)");
//    return factorial(n);
    BigInt ans;
...
//    const unsigned long q = uround_half_down(uabs(n), uabs(d)); // ROUND TOWARDS ZERO
  ...
  {
    if (IsNegative(n))
      CoCoA_ERROR(ERR::NotNonNegative, "FloorRoot: 1st arg must be non-negative");
    if (R < 1)
      CoCoA_ERROR(ERR::BadArg, "FloorRoot: 2nd arg must be positive");
    unsigned long r;
    if (!IsConvertible(r, R))
      CoCoA_ERROR(ERR::ArgTooBig, "FloorRoot: 2nd arg is too big"); // could just return 1???
    return FloorRoot(BigInt(n), r);
  }
  ...
  const BigInt FloorRoot(const BigInt& N, const MachineInt& r)
  {
    ...
    if (IsNegative(r) || IsZero(r))
      CoCoA_ERROR(ERR::BadArg, "FloorRoot: 2nd arg must be at least 1");
    BigInt ans;
  ...
  bool IsExactFloorRoot(long& ans, const MachineInt& n, const MachineInt& r)
  {
    ...
    if (!IsConvertible(ans, root))
      CoCoA_ERROR(ERR::ArgTooBig, "IsExactFloorRoot");
    return IsExact;
  }
  ...
  {
//    if (IsNegative(n))
//      CoCoA_ERROR(ERR::NotNonNegative, "IsExactFloorRoot: 1st arg must be non-negative");
    unsigned long r;
    if (!IsConvertible(r, R))
      CoCoA_ERROR(ERR::ArgTooBig, "IsExactFloorRoot: root index must be >=1 & fit into a ulong");
    BigInt root;
    const bool IsExact = IsExactFloorRoot(root, BigInt(n), r);
    if (!IsConvertible(ans, root))
      CoCoA_ERROR(ERR::ArgTooBig, "IsExactFloorRoot");
  ...
  bool IsExactFloorRoot(BigInt& ans, const BigInt& N, const MachineInt& r)
  {
    if (N < 0)
      CoCoA_ERROR(ERR::NotNonNegative, "IsExactFloorRoot: 1st arg must be non-negative");
    if (IsNegative(r) || IsZero(r))
      CoCoA_ERROR(ERR::BadArg, "IsExactFloorRoot: root index must be at least 1");
//    if (N < 0 && IsEven(AsUnsignedLong(r)))
//      CoCoA_ERROR(ERR::BadArg, "IsExactFloorRoot: even root of negative number");
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/BigIntOps.C,v 1.2 2019/09/16 14:30:34 abbott Exp $
// $Log: BigIntOps.C,v $
