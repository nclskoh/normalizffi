...
//       }
  ...
  {
    const ring& P = owner(f);
    if (!IsSparsePolyRing(P)) CoCoA_ERROR(ERR::NotSparsePolyRing, "IndetsProd");
  ...
  {
    if (L.empty()) CoCoA_ERROR(ERR::Empty, "IndetsProd");
  ...
  {
    if (L.empty()) CoCoA_ERROR(ERR::Empty, "IndetsIn");
    if (!IsSparsePolyRing(owner(L[0]))) CoCoA_ERROR(ERR::NotSparsePolyRing, "IndetsIn");
    const SparsePolyRing& P = owner(L[0]);
    const int nvars = NumIndets(P);
    const long n = len(L);
    for (long i=1; i < n; ++i)
      if (owner(L[i]) != P) CoCoA_ERROR(ERR::MixedRings, "IndetsIn");
  ...
  RingElem ConstantCoeff(ConstRefRingElem f)
  {
    ...
    const ring& P = owner(f);
    if (!IsSparsePolyRing(P))
      CoCoA_ERROR(ERR::NotSparsePolyRing, "ConstantCoeff(f)");
...
//      pos->second += monomial(P, coeff(it), PP(it)/t);
  ...
  std::vector<RingElem> CoeffVecWRTSupport(ConstRefRingElem f, ConstRefRingElem basis)
  {
    ...
    if (P != owner(basis)) CoCoA_ERROR(ERR::MixedRings, FnName);
    const ring& K = CoeffRing(P);
//???    if (!IsField(K)) CoCoA_ERROR(ERR::NotField, FnName);
    ...
    for (SparsePolyIter it=BeginIter(basis); !IsEnded(it); ++it, --i)
    {
      ...
      if (PPf != PP(it)) CoCoA_ERROR("not in span", FnName);
      C[i] = coeff(itf);
      ++itf;
    ...
    CoCoA_ERROR("not in span", FnName);
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/SparsePolyOps-RingElem.C,v 1.17 2020/04/19 17:15:07 abbott Exp $
