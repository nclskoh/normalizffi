...
//         return NewPolyRing(CoeffRing(P), NewSymbols(n+1), StdDegLex);
      CoCoA_ERROR(ERR::NYI, "GBasisByHomog: only for DegRevLex, lex");
...
// will be radical(I)
  ideal radical_tmp(const ideal& I)
  {
    ...
    if (AreGensMonomial(I)) return radical_MonId(I); // monomial ideal
    if (IsZeroDim(I)) return radical_0dim(I); // 0-dim ideal
    CoCoA_ERROR(ERR::NYI, "radical_tmp for general poly ideals");
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/SparsePolyOps-ideal.C,v 1.44 2020/03/06 18:40:07 abbott Exp $
