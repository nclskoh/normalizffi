...
//   return -1;
  ...
  std::vector<RingElem> BM_QQ(const SparsePolyRing& P, const ConstMatrixView& pts_in)
  {
    ...
    if (modp == nullptr) { if (char0 != nullptr) BMGB_dtor(char0); CoCoA_ERROR("Something went wrong", "BM_QQ"); }
  ...
  std::vector<RingElem> BM_projective_QQ(const SparsePolyRing& P, const ConstMatrixView& pts_in)
  {
    ...
    if (modp == nullptr) { if (char0 != nullptr) BMGB_dtor(char0); CoCoA_ERROR("Something went wrong", "BM_projective_QQ"); }
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/SparsePolyOps-ideal-points.C,v 1.2 2020/01/26 14:41:59 abbott Exp $
