...
namespace CoCoA
{
  ...
  namespace // anonymous for file local fn
  {
    ...
    void LogObsolescentFn(const char* const FnName, const char* const UsefulAdvice)
    {
      if (!IsAllowedObsolescentFnCall())
        CoCoA_ERROR(ERR::OBSOLESCENT, FnName + std::string(" -- ") + UsefulAdvice);
  ...
  const BigInt iroot(const MachineInt& n, const MachineInt& r)
  {
    ...
    if (IsNegative(n)) CoCoA_ERROR("iroot","1st arg must be non-negative");
  ...
  {
    LogObsolescentFn("iroot", "use \"FloorRoot\"");
    if (N < 0) CoCoA_ERROR("iroot","1st arg must be non-negative");
    return FloorRoot(N,r);
  }
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/obsolescent.C,v 1.11 2019/10/11 19:54:29 abbott Exp $
// $Log: obsolescent.C,v $
