...
// OBSOLETE -- new rref is "better"
  ...
  // {
  //   if (!IsField(RingOf(M_orig))) CoCoA_ERROR(ERR::NYI, "row reduction over non field");
  ...
  {
    if (!IsField(RingOf(M_orig))) CoCoA_ERROR(ERR::NotField, "RREFByGauss");
    matrix M = NewDenseMat(M_orig);
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/MatrixOps-rref.C,v 1.5 2020/02/12 10:38:56 abbott Exp $
