...
namespace CoCoA
{
  ...
  namespace  {  // anonymous
    ...
    {
      if (!IsCommutative(K))
        CoCoA_ERROR(ERR::NotCommutative, "NewPolyRing_DMP pseudo ctor");
    }
    ...
    {
      if (IndetNames.empty())
        CoCoA_ERROR(ERR::Empty, "NewPolyRing_DMPI pseudo ctor");
    ...
    {
      if (n <=0 )
        CoCoA_ERROR(ERR::BadNumIndets, "NewPolyRing_DMP pseudo ctor");
    }
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/RingDistrMPolyClean.C,v 1.26 2020/01/09 16:41:10 bigatti Exp $
