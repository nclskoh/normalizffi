...
//   } // anonymous end -------------------------------------------
  ...
  namespace  // anonymous
  {
    ...
    bool IsPrimary_0dimFin(const ideal& I)
    {
      ...
      if (!IsFiniteField(K))
        CoCoA_ERROR("CoeffRing must be finite field", "IsPrimary_0dimFin");
      if (!IsZeroDim(I))
  ...
  void SparsePolyRingBase::IdealImpl::myTestIsPrimary_0dim() const
  {
    ...
    {
      if (!IsFiniteField(CoeffRing(P)))
        CoCoA_ERROR(ERR::NYI, "only for finite fields");
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/SparsePolyOps-ideal-ZeroDim.C,v 1.8 2020/03/09 16:17:08 abbott Exp $
