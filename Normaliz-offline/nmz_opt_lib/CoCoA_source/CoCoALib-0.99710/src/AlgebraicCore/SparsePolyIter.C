...
namespace CoCoA
{
  ...
  SparsePolyIter BeginIter(ConstRefRingElem f)
  {
    if (!IsSparsePolyRing(owner(f)))
      CoCoA_ERROR(ERR::NotElemSparsePolyRing, "BeginIter(f)");
  ...
  {
    if (!IsSparsePolyRing(owner(f)))
      CoCoA_ERROR(ERR::NotElemSparsePolyRing, "EndIter(f)");
...
// $Header: /Volumes/Home_1/cocoa/cvs-repository/CoCoALib-0.99/src/AlgebraicCore/SparsePolyIter.C,v 1.1 2020/02/12 15:38:54 bigatti Exp $
//
